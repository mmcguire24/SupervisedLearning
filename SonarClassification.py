...
# load dataset
dataframe = pandas.read_csv("sonar.csv", header=0,encoding = "ISO-8859-1").dropna()
...
#lolloading
Xfull = dataset[:,0:60].astype(float)
Yfull = dataset[:,60]
...
def runSonarNN(params):
    ...
    #estimators.append(('mlp', KerasClassifier(build_fn=create_smaller, epochs=200, batch_size=5, verbose=0,learn_rate=.0001)))
  ...
  ####NOTE: Scikit Doesn't like Kerasclassier paramaters being passed in so i've just hardcoded the ones grid search found####
    mod = KerasClassifier(build_fn=create_smaller, epochs=200, batch_size=5, verbose=0,learn_rate=.0001)
    ...
    #title = "NN curve"
    #cv = ShuffleSplit(n_splits=5, test_size=0.2, random_state=0)
    #x = plot_learning_curve(pipeline, title, X, encoded_Y, cv=3, n_jobs=1)
    #x.show()
    return mod.score(Xval,encoded_Yval)
    #return numpy.mean(cross_val_score(pipeline, Xval, encoded_Yval, cv=5))
...
def GridSVM():
    tuned_parameters = [{'kernel': ['rbf','sigmoid','linear','poly'], 'gamma': [1e-3, 1e-4],
                         'C': [.1,1, 10,100,1000]}]
...
#print(clf)
#print("correct above 75",above75Right / (1.0* above75Right+above75Wrong))
#print("correct above 60",above60Right / (1.0* above60Right+above60Wrong))
#print ("wr 75",above75Right,above75Wrong)
#f = open("nntests.csv","a")
#acc = correct / (1.0*correct+wrong)
#above75per = above75Right / (1.0* above75Right+above75Wrong)
#above60per = above60Right / (1.0* above60Right+above60Wrong)
#print(200.0*(above60Wrong+above60Wrong) / len(Xval))
#print(200.0*(above75Wrong+above75Wrong) / len(Xval))
...
def GridDT():
    ...
    parameter_grid = {'max_depth': [i for i in range(1,50)],
                  'max_features': [i for i in range(1,50)]}
...
def GridSearch(m,tuned_parameters):
    ...
    for score in scores:
        ...
        clf = GridSearchCV(m, tuned_parameters, cv=5,
                           scoring='%s_macro' % score)
        clf.fit(X, encoded_Y)
